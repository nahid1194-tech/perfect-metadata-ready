...
- **Routing**: `src/routes/adobe.ts` — `GET /api/adobe/search?creatorId=…` (primary Creator search; calls the official Adobe Stock API) and `GET /api/adobe/creator/:creatorId` (same lookup in the normalized §18 wire shape). `src/routes/creator.ts` — `GET /api/creator/:creatorId/assets`, `GET /api/creator/:creatorId/stats`, `GET /api/creator/:creatorId/overview`, `GET /api/creator/:creatorId/availability`, and `GET /api/creator/:creatorId/links`. `src/routes/search.ts` — `GET /api/search/links` and `GET /api/search/assets` (both take `q`; the asset endpoint also takes `filter`/`sort`/`page`/`limit`). `src/routes/assets.ts` — `GET /api/assets/:assetId`, `GET /api/assets/:assetId/history`, `GET /api/assets/:assetId/links`. `src/routes/analytics.ts` — `GET /api/analytics/summary` and `GET /api/analytics/keywords`. `src/routes/licenseHistory.ts` — `GET /api/license-history`. Query params are validated in `src/lib/validation.ts`; Creator/Asset IDs must be numeric, search phrases must be 1–300 chars, and license-history dates must be `YYYY-MM-DD`.
